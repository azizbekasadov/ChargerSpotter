//
//  MockCoreDataStorageService.swift
//  ChargerSpotter
//
...
final class MockCoreDataStorageService: StorageService {
    ...
    private func createRequest<T: Storagable>(
        withResultType type: T.Type
    ) throws -> NSFetchRequest<T> {
    ...
    func removeBatch<T>(type: T.Type) where T : Storagable {
        ...
        do {
            ...
            testLogger.error(.init(stringLiteral: error.localizedDescription))
        }
    }
}
